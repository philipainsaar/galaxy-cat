...
function createMovingBackgroundSymbols() {
  ...
  for (let i = 0; i < TOTAL_MOVING_SYMBOLS; i += 1) {
    const src = MOVING_BG_SYMBOLS[i % MOVING_BG_SYMBOLS.length];

    // Higher number = more symbols near the top of the image.
    const topBias = Math.pow(Math.random(), 3.2);
    ...
    symbols.push({
      ...
      driftX: Math.random() * 64 - 32,
      driftY: Math.random() * 30 - 6,

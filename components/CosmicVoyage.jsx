...
function createInfinityTriangleWater() {
  // A separate far-water extension only after the old waves.
  // It is a real triangle that points toward the top of the screen / infinity.
  // The old high-wave water stays fully opaque and cannot show this through it.
  ...
  const material = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        // Triangle-mask fade: soft sides, visible base, pointed infinity tip.
        float side = 1.0 - abs(uv.x - 0.5) * 2.0;
        float sideFade = smoothstep(0.02, 0.18, side);
        float baseFade = smoothstep(0.00, 0.12, uv.y);
        float tipFade = 1.0 - smoothstep(0.88, 1.0, uv.y);

        // Horizontal-only drifting reflections.
        float a = sin(uv.x * 34.0 + uTime * 1.25 + uv.y * 4.0);
        float b = sin(uv.x * 70.0 - uTime * 1.85 + uv.y * 2.0);
        float c = sin(uv.x * 130.0 + uTime * 0.75);
        float water = a * 0.45 + b * 0.25 + c * 0.12;
        water = water * 0.5 + 0.5;

        vec3 blue = vec3(0.72, 0.93, 1.0);
        vec3 pink = vec3(1.0, 0.78, 0.96);
        vec3 lavender = vec3(0.84, 0.80, 1.0);
        vec3 white = vec3(1.0, 0.98, 1.0);

        vec3 color = mix(blue, pink, water);
        color = mix(color, lavender, uv.y * 0.34);

        float gloss = smoothstep(0.72, 0.98, water);
        color = mix(color, white, gloss * 0.38);

        float alpha = sideFade * baseFade * tipFade * 0.82;
  ...
  mesh.name = 'InfinityTrianglePastelWater';
  mesh.renderOrder = 0;
...
export default function CosmicVoyage() {
  ...
  useEffect(() => {
    ...
    scene.add(infinityTriangleWater.mesh);
    ...
    renderer.setAnimationLoop(() => {
      ...
      // Far triangle water only after the opaque old waves.
      // The old water remains non-transparent, so this cannot show through it.

...
function createFurNoiseTexture() {
  ...
  // Tiny random hair strokes. This is only a texture, not extra model polygons.
  for (let i = 0; i < 1900; i += 1) {
    const x = Math.random() * size;
    const y = Math.random() * size;
    const length = 2 + Math.random() * 9;
    const angle = Math.random() * Math.PI * 2;
    const alpha = 0.035 + Math.random() * 0.22;

    ctx.strokeStyle = `rgba(255,255,255,${alpha})`;
    ctx.lineWidth = 0.35 + Math.random() * 1.15;
    ctx.beginPath();
    ctx.moveTo(x, y);
    ctx.lineTo(
...
function createFurShellMaterial({
  ...
  return new THREE.ShaderMaterial({
    ...
    vertexShader: `
      ...
      void main() {
        ...
        float sway = sin(position.x * 6.0 + position.y * 4.0 + shell * 8.0) * 0.006;
        vec3 pushedPosition = position + normal * (uLength * shell + sway * shell);
    ...
    fragmentShader: `
      ...
      void main() {
        float shell = uLayer / uLayers;
        vec2 movingUv = vUv * 5.5 + vec2(uTime * 0.018, shell * 0.23);
        float hair = texture2D(uFurTexture, movingUv).a;

        // More visible at the silhouette, softer on flat front-facing areas.
        float facing = abs(dot(normalize(vNormalView), vec3(0.0, 0.0, 1.0)));
        float rim = pow(1.0 - facing, 0.55);
        float bodyFuzz = 0.26 * hair;
        float silhouetteFuzz = hair * rim;

        float alpha = (bodyFuzz + silhouetteFuzz) * uOpacity;
        alpha *= 1.0 - shell * 0.34;

        if (alpha < 0.016) discard;

        vec3 color = mix(uColor, vec3(1.0), 0.24 + rim * 0.18);
...
function shouldSkipFurMesh(object, skipPattern) {
  ...
  // Keep eyes, black pupils, glass and transparent details clean.
  return materials.some((material) => {
    if (!material) return false;
    if (material.transparent && material.opacity < 0.75) return true;

    if (!material.color) return false;
    const { r, g, b } = material.color;
    const luminance = r * 0.2126 + g * 0.7152 + b * 0.0722;

    return luminance < 0.07;
  });
}
...
function addFluffyShellsToModel(root, options = {}) {
  const {
    ...
    furTexture,
    renderOrderBase = 80,
    skipPattern = /eye|eyes|pupil|pupils|glass|window|windows|light|glow|black|mouth|teeth|propeller|wheel/i,
  } = options;
  ...
  meshes.forEach((mesh) => {
    ...
    const baseColor = averageMaterialColor(materials, fallbackColor)
      .lerp(fallbackColor, 0.32);

    for (let layer = 1; layer <= layers; layer += 1) {
      const material = createFurShellMaterial({
      ...
      shell.name = `${mesh.name || 'mesh'}_pastel_fur_shell_${layer}`;
      shell.frustumCulled = false;
      shell.renderOrder = renderOrderBase + layer;
...
export default function CosmicVoyage() {
  ...
  useEffect(() => {
    ...
    const furTexture = createFurNoiseTexture();
    const furMaterials = [];
      ...
      .then(([boatGLTF, catGLTF]) => {
        ...
        furMaterials.push(
          ...addFluffyShellsToModel(boatModel, {
            furTexture,
            layers: window.innerWidth < 768 ? 4 : 5,
            length: 0.034,
            opacity: 0.16,
            tint: '#ffe8ff',
            renderOrderBase: 60,
            skipPattern:
              /eye|eyes|pupil|pupils|glass|window|windows|light|glow|black|mouth|teeth|propeller|wheel|rim/i,
          }),
        );
        ...
        furMaterials.push(
          ...addFluffyShellsToModel(catModel, {
            furTexture,
            layers: window.innerWidth < 768 ? 7 : 9,
            length: 0.068,
            opacity: 0.28,
            tint: '#efff70',
            renderOrderBase: 90,
            skipPattern:
              /eye|eyes|pupil|pupils|glass|window|windows|light|glow|black|mouth|teeth/i,
          }),
        );

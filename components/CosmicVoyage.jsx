...
function createTopPastelWaterLayer() {
  // Extra soft pastel water that sits ABOVE the far/top end of the old water.
  // This is intentionally not a replacement for the old wave mesh.
  const geometry = new THREE.PlaneGeometry(58, 20, 2, 2);
  ...
  const material = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        // Horizontal drifting reflection only.
        float a = sin(uv.x * 24.0 + uTime * 1.15 + uv.y * 1.4);
        float b = sin(uv.x * 46.0 - uTime * 0.85 + uv.y * 2.0);
        float c = sin(uv.x * 82.0 + uTime * 0.55);
        float water = (a * 0.50 + b * 0.32 + c * 0.18) * 0.5 + 0.5;

        vec3 blue = vec3(0.72, 0.94, 1.0);
        vec3 pink = vec3(1.0, 0.80, 0.97);
        vec3 lavender = vec3(0.86, 0.84, 1.0);
        vec3 white = vec3(1.0, 0.985, 1.0);

        vec3 color = mix(blue, pink, water);
        color = mix(color, lavender, 0.20);

        float gloss = smoothstep(0.76, 1.0, water);
        color = mix(color, white, gloss * 0.50);

        // Fade edges so it only appears as a soft far/top water sheet.
        float edgeX = smoothstep(0.0, 0.12, uv.x) * (1.0 - smoothstep(0.88, 1.0, uv.x));
        float edgeY = smoothstep(0.0, 0.12, uv.y) * (1.0 - smoothstep(0.78, 1.0, uv.y));
        float alpha = edgeX * edgeY * 0.72;
  ...
  const water = new THREE.Mesh(geometry, material);
  water.name = 'TopPastelWaterLayer';
  water.rotation.x = -Math.PI / 2;

  // Higher Y = vertically higher on screen. Farther Z = at the top/back end of the old water.
  water.position.set(0, -0.22, -16.5);
  water.renderOrder = 4;
...
export default function CosmicVoyage() {
  ...
  useEffect(() => {
    ...
    const topPastelWater = createTopPastelWaterLayer();
    scene.add(topPastelWater);
    ...
    renderer.setAnimationLoop(() => {
      ...
      topPastelWater.material.uniforms.uTime.value = elapsed;

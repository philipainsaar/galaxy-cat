...
// Fast circular background: one 21:9 tile is repeated around a 3D cylinder.
// The cylinder height is calculated from circumference / repeats / (21 / 9),
// so the image is not squashed into the phone screen.
const SKY_TEXTURE_URL = '/images/pastel-sky.jpg';
const SKY_IMAGE_ASPECT = 21 / 9;
const SKY_REPEAT_AROUND = 4;
const SKY_RADIUS = 140;
const SKY_TEXTURE_SCROLL_SPEED = 0.42;
const SKY_CYLINDER_SPIN_SPEED = 0.32;
...
function createFurNoiseTexture() {
  ...
  for (let i = 0; i < 4200; i += 1) {
    ...
    ctx.strokeStyle = `rgba(255,255,255,${0.10 + Math.random() * 0.46})`;
    ctx.lineWidth = 0.55 + Math.random() * 1.45;
    ctx.beginPath();
    ctx.moveTo(x, y);
    ctx.lineTo(
...
function createSoftFluffTexture() {
  const size = 128;
  const canvas = document.createElement('canvas');
  canvas.width = size;
  canvas.height = size;
  ...
  for (let i = 0; i < 220; i += 1) {
    const angle = Math.random() * Math.PI * 2;
    const start = 6 + Math.random() * 15;
    const end = 34 + Math.random() * 31;

    ctx.strokeStyle = `rgba(255,255,255,${0.12 + Math.random() * 0.28})`;
    ctx.lineWidth = 0.45 + Math.random() * 1.1;
    ctx.beginPath();
...
function ensureFurUvAttribute(geometry) {
  ...
  for (let i = 0; i < position.count; i += 1) {
    ...
    uv[i * 2] = size.x > 0 ? (x - box.min.x) / size.x : 0.5;
    uv[i * 2 + 1] = size.z > 0 ? (z - box.min.z) / size.z : 0.5;
  }

  geometry.setAttribute('uv', new THREE.BufferAttribute(uv, 2));
}

function materialLooksDark(material) {
  if (!material?.color) return false;
  return material.color.r + material.color.g + material.color.b < 0.18;
}

function shouldSkipFurMesh(object, skipPattern) {
  ...
  if (skipPattern.test(combinedName)) return true;

  return materials.some(materialLooksDark);
}

function averageMaterialColor(materials, fallbackColor) {
  const color = fallbackColor.clone();
  let found = 0;

  materials.forEach((material) => {
    if (!material?.color) return;
    color.lerp(material.color, found === 0 ? 1 : 1 / (found + 1));
    found += 1;
  });

  return found > 0 ? color : fallbackColor.clone();
}

function createFluffyShellMaterial({
  ...
  return new THREE.ShaderMaterial({
    ...
    vertexShader: `
      ...
      void main() {
        ...
        float shell = uLayer / uLayers;
        float fuzzSway = sin(position.x * 5.3 + position.y * 6.7 + uTime * 2.2 + shell * 8.0) * 0.010;
        vec3 pushedPosition = position + normal * (uLength * shell + fuzzSway * shell);
    ...
    fragmentShader: `
      ...
      void main() {
        float shell = uLayer / uLayers;
        vec2 movingUv = vUv * 7.0 + vec2(uTime * 0.045, shell * 0.37);
        float hair = texture2D(uFurTexture, movingUv).a;

        float facing = abs(dot(normalize(vNormalView), vec3(0.0, 0.0, 1.0)));
        float rim = pow(1.0 - facing, 0.36);
        float body = 0.24 + hair * 1.32;
        float outline = rim * (0.95 + hair * 1.25);
        float alpha = (body + outline) * uOpacity * (1.0 - shell * 0.20);

        if (alpha < 0.018) discard;

        vec3 softWhite = vec3(1.0, 0.98, 1.0);
        vec3 finalColor = mix(uColor, softWhite, 0.24 + rim * 0.42);
...
function addFluffyShellsToModel(root, options = {}) {
  const {
    furTexture,
    layers = 8,
    length = 0.06,
    opacity = 0.28,
    tint = '#ffffff',
    skipPattern = /eye|eyes|pupil|iris|glass|window|light|glow|black|mouth|teeth/i,
  } = options;

  if (!furTexture) return [];

  const meshes = [];
  root.traverse((object) => {
    if (!object.isMesh || !object.geometry) return;
    if (shouldSkipFurMesh(object, skipPattern)) return;
    meshes.push(object);
  });
  ...
  meshes.forEach((mesh) => {
    ensureFurUvAttribute(mesh.geometry);
    ...
    const baseColor = averageMaterialColor(materials, tintColor).lerp(tintColor, 0.38);

    for (let layer = 1; layer <= layers; layer += 1) {
      const material = createFluffyShellMaterial({
        color: baseColor,
        texture: furTexture,
        layer,
        layers,
      ...
      shell.name = `${mesh.name || 'mesh'}_very_visible_fur_shell_${layer}`;
      shell.frustumCulled = false;
      shell.renderOrder = 90 + layer;
      shell.userData.isFurShell = true;

      mesh.add(shell);
      furMaterials.push(material);
    }
  });
...
function collectSurfaceSamples(root, skipPattern) {
  const samples = [];
  const rootInverse = new THREE.Matrix4();
  const tempLocal = new THREE.Vector3();
  const tempWorld = new THREE.Vector3();
  const tempNormal = new THREE.Vector3();
  const tempNormalTip = new THREE.Vector3();
  ...
  root.traverse((object) => {
    ...
    const step = Math.max(1, Math.floor(positionAttr.count / 1000));
    normalMatrix.getNormalMatrix(object.matrixWorld);

    for (let i = 0; i < positionAttr.count; i += step) {
      tempLocal.fromBufferAttribute(positionAttr, i);
      tempWorld.copy(tempLocal).applyMatrix4(object.matrixWorld);

      const rootPosition = tempWorld.clone().applyMatrix4(rootInverse);
      ...
      tempNormalTip.copy(tempWorld).add(tempNormal).applyMatrix4(rootInverse);
      const rootNormal = tempNormalTip.sub(rootPosition).normalize();

      samples.push({
        position: rootPosition.clone(),
        normal: rootNormal.lengthSq() > 0.01 ? rootNormal.clone() : new THREE.Vector3(0, 1, 0),
      });
    }
  });
...
function addSurfaceFurPointCloud(root, options = {}) {
  const {
    texture,
    count = 2500,
    size = 0.18,
  ...
  const samples = collectSurfaceSamples(root, skipPattern);
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    const out = shellOffset * (0.38 + Math.random() * 1.25);
    ...
    const point = sample.position
      .clone()
      .addScaledVector(sample.normal, out)
      .add(randomDirection);
  ...
  cloud.name = `${root.name || 'model'}_visible_surface_fur_cloud`;
  cloud.frustumCulled = false;
  cloud.renderOrder = 130;
  cloud.userData.fluff = {
    baseSize: size,
    baseOpacity: opacity,
...
function createFastCircularSky(renderer) {
  const tileWidth = (Math.PI * 2 * SKY_RADIUS) / SKY_REPEAT_AROUND;
  const cylinderHeight = tileWidth / SKY_IMAGE_ASPECT;
  ...
  const texture = new THREE.TextureLoader().load(SKY_TEXTURE_URL);
  texture.colorSpace = THREE.SRGBColorSpace;
  texture.wrapS = THREE.RepeatWrapping;
  texture.wrapT = THREE.ClampToEdgeWrapping;
  texture.repeat.set(SKY_REPEAT_AROUND, 1);
  ...
  cylinder.name = 'FastCircularTwentyOneByNinePastelSkyNoStretch';
  cylinder.rotation.y = Math.PI;
  cylinder.frustumCulled = false;
  cylinder.renderOrder = -200000;

  const group = new THREE.Group();
  group.name = 'Fast360CircularPastelSkyGroup';
  group.renderOrder = -200000;
  group.add(cylinder);
...
export default function CosmicVoyage() {
  ...
  useEffect(() => {
    ...
    // Real 360° / circular 21:9 background.
    // It is a cylinder around the camera, not a stretched CSS background.
    const circularSky = createFastCircularSky(renderer);
    circularSky.group.position.copy(camera.position);
    scene.add(circularSky.group);

    // Obvious low-poly fluffy effect: shader shells + point-cloud fuzz.
    const furNoiseTexture = createFurNoiseTexture();
    const softFluffTexture = createSoftFluffTexture();
    const furMaterials = [];
    const furClouds = [];
  ...
  (texture) => {
    ...
    Promise.all([
      ...
      .then(([boatGLTF, catGLTF]) => {
        ...
        furMaterials.push(
          ...addFluffyShellsToModel(boatModel, {
            furTexture: furNoiseTexture,
            layers: window.innerWidth < 768 ? 6 : 8,
            length: 0.055,
            opacity: 0.26,
            tint: '#fff0ff',
            skipPattern: /eye|eyes|pupil|iris|glass|window|light|glow|black|mouth|teeth|propeller/i,
          }),
        );

        const boatFurCloud = addSurfaceFurPointCloud(boatModel, {
          texture: softFluffTexture,
          count: window.innerWidth < 768 ? 2200 : 3600,
          size: window.innerWidth < 768 ? 0.17 : 0.20,
          opacity: 0.62,
          shellOffset: 0.075,
          jitter: 0.050,
          palette: ['#bdf6ff', '#ffd0f5', '#d7c8ff', '#ffffff'],
          skipPattern: /eye|eyes|pupil|iris|glass|window|light|glow|black|mouth|teeth|propeller/i,
          spinSpeed: 0.018,
        });

        if (boatFurCloud) furClouds.push(boatFurCloud);
        ...
        furMaterials.push(
          ...addFluffyShellsToModel(catModel, {
            furTexture: furNoiseTexture,
            layers: window.innerWidth < 768 ? 8 : 10,
            length: 0.105,
            opacity: 0.40,
            tint: '#eeff70',
            skipPattern: /eye|eyes|pupil|iris|glass|window|light|glow|black|mouth|teeth/i,
        ...
        const catFurCloud = addSurfaceFurPointCloud(catModel, {
          texture: softFluffTexture,
          count: window.innerWidth < 768 ? 3000 : 5200,
          size: window.innerWidth < 768 ? 0.18 : 0.22,
          opacity: 0.78,
          shellOffset: 0.115,
          jitter: 0.070,
          palette: ['#eeff70', '#dfff8c', '#fffbd8', '#ffffff'],
          skipPattern: /eye|eyes|pupil|iris|glass|window|light|glow|black|mouth|teeth/i,
          spinSpeed: 0.035,
        });

        if (catFurCloud) furClouds.push(catFurCloud);
    ...
    renderer.setAnimationLoop(() => {
      ...
      // Fast 360° loop: rotate the inside cylinder and scroll the 21:9 texture.
      circularSky.group.position.copy(camera.position);
      circularSky.texture.offset.x =
        (circularSky.texture.offset.x + delta * SKY_TEXTURE_SCROLL_SPEED) % 1;
      circularSky.cylinder.rotation.y += delta * SKY_CYLINDER_SPIN_SPEED;

      furMaterials.forEach((material) => {
        if (material.uniforms?.uTime) {
          material.uniforms.uTime.value = elapsed;
        }
      });

      furClouds.forEach((cloud) => {
        const base = cloud.userData.fluff;
        if (!base) return;

        const pulse = Math.sin(elapsed * 2.8 + base.phase) * 0.5 + 0.5;
        cloud.material.opacity = base.baseOpacity * (0.82 + pulse * 0.18);
        cloud.material.size = base.baseSize * (0.92 + pulse * 0.16);
        cloud.rotation.y += delta * base.spinSpeed;
        cloud.rotation.x = Math.sin(elapsed * 1.7 + base.phase) * 0.012;
      });

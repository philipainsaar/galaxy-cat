...
function improveModelQuality(root, renderer, pastelPalette) {
  ...
  root.traverse((object) => {
    ...
    const tint = pastelPalette?.[meshIndex % pastelPalette.length];
    meshIndex += 1;
    ...
    materials.forEach((material) => {
      ...
      if (tint && material.color) {
        material.color.lerp(tint, material.map ? 0.18 : 0.42);
      }
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([boatGLTF, catGLTF]) => {
        ...
        dropZoneRing.position.y = seatPosition.y + CAT_GROUND_Y + 0.04;
        ...
        const hitRadius = Math.max(catSize.x, catSize.y, catSize.z) * 0.7;
    ...
    const burstPoints = new THREE.Points(particleGeometry, particleMaterial);
    ...
    const pointerToWorld = (clientX, clientY) => {
      ...
      raycaster.ray.intersectPlane(dragPlane, temporaryWorldPosition);
    ...
    const tryDrag = (clientX, clientY) => {
      ...
      const modelIntersections = raycaster.intersectObject(catGroup, true);
      const catWorldPosition = catGroup.getWorldPosition(new THREE.Vector3());

      const closeToCat =
        raycaster.ray.distanceToPoint(catWorldPosition) < state.catHitRadius;
    ...
    const moveDraggedCat = (clientX, clientY) => {
      ...
      state.dragDistancePx = Math.max(state.dragDistancePx, distance);
      ...
      if (worldPosition.lengthSq() > 0.001) {
        state.tgt.set(worldPosition.x, worldPosition.y, DRAG_Z);
      }
    };
    ...
    const finishDrag = (clientX, clientY) => {
      ...
      if (releasedOverBoat) {
        ...
        state.tgt.copy(boatGroup.localToWorld(seatPosition.clone()));
        ...
        const progressTimer = window.setInterval(() => {
          const nextPercent = Math.min(
            100,
            Math.round(((Date.now() - startedAt) / 3000) * 100),
          );
    ...
    renderer.setAnimationLoop(() => {
      ...
      for (let i = 0; i < pastelWater.waterPositions.count; i += 1) {
        ...
        const longWave = Math.sin(x * 0.18 + elapsed * 2.55) * 0.50;
        const crossWave = Math.cos(y * 0.16 + elapsed * 2.15) * 0.36;
        const diagonalWave = Math.sin((x + y) * 0.085 + elapsed * 3.2) * 0.22;
        const smallTide = Math.cos((x - y) * 0.12 + elapsed * 4.1) * 0.12;
        const heightValue = longWave + crossWave + diagonalWave + smallTide;

        pastelWater.waterPositions.array[i * 3 + 2] = heightValue;

        const crest = THREE.MathUtils.clamp((heightValue + 0.75) / 1.65, 0, 1);
        const ribbonA = (Math.sin(x * 0.105 + elapsed * 0.9) + 1) * 0.5;
        const ribbonB = (Math.cos(y * 0.115 - elapsed * 0.7) + 1) * 0.5;
        const ribbonC = (Math.sin((x + y) * 0.06 + elapsed * 1.15) + 1) * 0.5;
      ...
      if (state.launching) {
        ...
        const launchProgress = state.launchElapsed / LAUNCH_DURATION_SECONDS;
        const easedLaunch = easeOutCubic(launchProgress);

        // Turn the boat around and send it far into the pastel distance.
        boatGroup.position.x = Math.sin(easedLaunch * Math.PI) * 1.6;
        boatGroup.position.y =
          BOAT_WATERLINE_Y +
          easedLaunch * LAUNCH_HEIGHT +
          Math.sin(elapsed * 8.0) * 0.08;
        boatGroup.position.z = BOAT_DEPTH - easedLaunch * LAUNCH_DISTANCE;
      ...
      dropZoneMaterial.opacity = lerp(
        ...
        dragging ? 0.5 + Math.sin(elapsed * 4) * 0.2 : 0,
        Math.min(1, delta * 7),
      );
      ...
      const followAmount = Math.min(1, delta * followSpeed);

      if (state.landed && !state.launchComplete) {
        // Convert the seat marker/local seat position into scene space
        // every frame so the cat remains locked to the flying boat.
        boatGroup.updateMatrixWorld(true);
        state.tgt.copy(boatGroup.localToWorld(seatPosition.clone()));
      }

      state.cur.x = lerp(state.cur.x, state.tgt.x, followAmount);
      state.cur.y = lerp(state.cur.y, state.tgt.y, followAmount);
      state.cur.z = lerp(state.cur.z, state.tgt.z, followAmount);
      ...
      if (state.landed && !state.launchComplete) {
        const happyPulse = Math.max(0, Math.sin(elapsed * 8));
      ...
      if (state.burstT >= 0) {
        ...
        particleMaterial.opacity =
          state.burstT < 1.6 ? Math.max(0, 1 - state.burstT / 1.6) : 0;
      ...
      {modelError && (
        <div className="popupWindow">
          <div className="termHeader">
            ...
            <button type="button" onClick={() => setModelError('')}>
              CLOSE
            </button>
          </div>

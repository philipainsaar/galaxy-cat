...
function createInfinityTriangleWater() {
  // Background infinity triangle. It is drawn FIRST, like z-index: -100000.
  // It is not transparent, so the old opaque waves, boat and cat always cover it.
  ...
  const material = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        // Keep a clean triangle, but fade the visible edge into the white background
        // using color instead of alpha. This stops it from rendering over the old waves.
        float edgeFade = smoothstep(0.0, 0.075, uv.x) * smoothstep(1.0, 0.925, uv.x);
        float baseFade = smoothstep(0.0, 0.22, uv.y);
        float tipFade = 1.0 - smoothstep(0.92, 1.0, uv.y) * 0.35;
        float shape = edgeFade * baseFade * tipFade;

        if (shape < 0.015) discard;

        // Sideways-only pastel water reflections.
        ...
        vec3 blue = vec3(0.78, 0.93, 1.0);
        vec3 lavender = vec3(0.86, 0.82, 1.0);
        vec3 pink = vec3(1.0, 0.84, 0.98);
        vec3 white = vec3(1.0, 0.98, 1.0);

        vec3 color = mix(blue, lavender, uv.y * 0.70);
        color = mix(color, pink, water * 0.20);

        float centerGlow = 1.0 - abs(uv.x - 0.5) * 2.0;
        centerGlow = pow(max(centerGlow, 0.0), 2.4);
        color = mix(color, white, centerGlow * 0.20);

        float gloss = smoothstep(0.72, 1.0, water) * (1.0 - uv.y * 0.45);
        color = mix(color, white, gloss * 0.22);

        // Soft visual fade to white, but still opaque for render ordering.
        color = mix(bg, color, shape * 0.72);
  ...
  triangle.name = 'InfinityTrianglePastelWaterZIndexMinus100000';

  // Far behind everything, higher on screen, pointing upward to infinity.
  triangle.position.set(0, 0.6, -34.0);

...
function collectFallbackFluffSamples(root, count = 260) {
  const samples = [];
  const box = new THREE.Box3().setFromObject(root);

  if (box.isEmpty()) return samples;
  ...
  const worldNormal = new THREE.Vector3();
  const rootNormalTip = new THREE.Vector3();

  for (let i = 0; i < count; i += 1) {
    const angle = Math.random() * Math.PI * 2;
    const height = (Math.random() - 0.5) * sizeWorld.y;
    const radiusX = sizeWorld.x * (0.35 + Math.random() * 0.18);
    const radiusZ = sizeWorld.z * (0.35 + Math.random() * 0.18);

    worldPosition.set(
      centerWorld.x + Math.cos(angle) * radiusX,
      centerWorld.y + height,
      centerWorld.z + Math.sin(angle) * radiusZ,
    );

    worldNormal.copy(worldPosition).sub(centerWorld).normalize();
    if (worldNormal.lengthSq() < 0.001) worldNormal.set(0, 1, 0);

    rootPosition.copy(worldPosition).applyMatrix4(rootInverse);
    rootNormalTip.copy(worldPosition).add(worldNormal).applyMatrix4(rootInverse);

    samples.push({
      position: rootPosition.clone(),
      normal: rootNormalTip.sub(rootPosition).normalize(),
    });
  }
...
function addTexturePreservingFluff(root, options = {}) {
  const {
    ...
    jitter = 0.055,
    palette = ['#ffffff'],
    skipPattern = /eye|eyes|pupil|iris|glass|window|mouth|teeth|metal|chrome|propeller/i,
  } = options;
  ...
  // Some GLB exports have material names that accidentally match the skip list.
  // Fallback samples make the fluff visible instead of silently returning null.
  if (!samples.length) {
    samples = collectFluffSurfaceSamples(root, /eye|eyes|pupil|iris|glass|window/i);
  }

  if (!samples.length) {
    samples = collectFallbackFluffSamples(root, Math.min(360, count));
  }
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    const distance = shellOffset * (0.75 + Math.random() * 1.7);
    ...
    const color = colorPalette[i % colorPalette.length]
      .clone()
      .lerp(new THREE.Color('#ffffff'), 0.22 + Math.random() * 0.42);
  ...
  cloud.name = `${root.name || 'model'}_visible_texture_preserving_fluff`;
  ...
  (texture) => {
    ...
    Promise.all([
      ...
      .then(([boatGLTF, catGLTF]) => {
        ...
        const boatFluff = addTexturePreservingFluff(boatModel, {
          ...
          count: width < 768 ? 560 : 820,
          size: width < 768 ? 0.18 : 0.14,
          opacity: 0.62,
          shellOffset: 0.13,
          jitter: 0.06,
        ...
        const catFluff = addTexturePreservingFluff(catModel, {
          texture: fluffTexture,
          count: width < 768 ? 920 : 1320,
          size: width < 768 ? 0.16 : 0.125,
          opacity: 0.78,
          shellOffset: 0.15,
          jitter: 0.07,
          palette: ['#fff6fb', '#f5e9ff', '#e8fbff', '#ffffff'],
          skipPattern: /eye|eyes|pupil|iris|glass|window|mouth|teeth|metal|chrome/i,
        });
    ...
    renderer.setAnimationLoop(() => {
      ...
      fluffClouds.forEach((cloud) => {
        ...
        cloud.material.opacity = motion.baseOpacity * (0.82 + Math.sin(elapsed * 1.75 + motion.phase) * 0.18);
        cloud.rotation.y = Math.sin(elapsed * 0.72 + motion.phase) * 0.026;
        cloud.rotation.x = Math.cos(elapsed * 0.55 + motion.phase) * 0.018;
      });

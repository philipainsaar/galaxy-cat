...
function createInfinityTriangleWater() {
  // Background infinity triangle like a pastel water/mountain path.
  // It is a separate vertical plane behind the boat and old water.
  // The old water remains fully opaque and hides the lower part naturally.
  ...
  const material = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        // Triangle is widest at the old water horizon and points upward to infinity.
        float sideFade = smoothstep(0.0, 0.08, uv.x) * smoothstep(1.0, 0.92, uv.x);
        float topGlow = smoothstep(1.0, 0.35, uv.y);
        float bottomFade = smoothstep(0.0, 0.12, uv.y);

        // Sideways-only soft reflection motion.
        float driftA = sin(uv.x * 18.0 + uTime * 0.65 + uv.y * 2.0);
        float driftB = sin(uv.x * 34.0 - uTime * 0.9 + uv.y * 4.0);
        float driftC = sin(uv.x * 9.0 + uTime * 0.35);
        float water = driftA * 0.34 + driftB * 0.16 + driftC * 0.10;
        water = water * 0.5 + 0.5;

        vec3 blue = vec3(0.76, 0.92, 1.0);
        vec3 lavender = vec3(0.86, 0.82, 1.0);
        vec3 pink = vec3(1.0, 0.83, 0.98);
        vec3 white = vec3(1.0, 0.98, 1.0);

        vec3 color = mix(blue, lavender, uv.y * 0.72);
        color = mix(color, pink, water * 0.22);

        float centerGlow = 1.0 - abs(uv.x - 0.5) * 2.0;
        centerGlow = pow(max(centerGlow, 0.0), 2.5);
        color = mix(color, white, centerGlow * 0.22);

        float gloss = smoothstep(0.72, 1.0, water) * (1.0 - uv.y * 0.55);
        color = mix(color, white, gloss * 0.26);

        float alpha = sideFade * bottomFade * topGlow * 0.70;

        // Softer transparent tip, still visibly pointing upward.
        alpha *= mix(1.0, 0.35, smoothstep(0.76, 1.0, uv.y));
  ...
  triangle.name = 'InfinityTrianglePastelWaterBehindBoat';

  // Behind the boat and old waves, high enough to appear above the old water horizon.
  triangle.position.set(0, -0.12, -10.8);
...
export default function CosmicVoyage() {
  ...
  useEffect(() => {
    ...
    // Infinity triangle water is behind the boat and above the old wave horizon.
    // The old high waves stay fully opaque and hide the lower part of this triangle.

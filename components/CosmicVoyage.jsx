...
function createMovingBackgroundSymbols() {
  ...
  // These rows keep the symbols mostly near the top of pastel-sky.jpg,
  // with enough vertical breathing room so they do not sit on top of each other.
  const topWeightedRows = [7, 16, 27, 39, 52, 66];
  ...
  for (let row = 0; row < rows; row += 1) {
    for (let column = 0; column < columns; column += 1) {
      gridSpots.push({
        ...
        y: topWeightedRows[row],
      });
    }
  }

  return shuffleArray(gridSpots)
    .slice(0, TOTAL_MOVING_SYMBOLS)
    .map((spot, index) => {
      const src = MOVING_BG_SYMBOLS[index % MOVING_BG_SYMBOLS.length];

      return {
        id: `symbol-${index}`,
        src,

        // Small jitter keeps the layout organic while preserving spacing.
        x: spot.x + (Math.random() * 5 - 2.5),
        y: spot.y + (Math.random() * 4 - 2),
...
if (state.landed) {
  ...
  return (
    <main className="stage">
      <div className="pastelBackgroundMotion" aria-hidden="true">

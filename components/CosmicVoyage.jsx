...
function createInfinityTriangleWater() {
  // Far/top water extension: a transparent triangular shader plane
  // that visually narrows into the distance without changing the old water.
  const geometry = new THREE.PlaneGeometry(42, 70, 1, 1);
  ...
  const material = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        // uv.y = 0 is the far/sharp end, uv.y = 1 is the wide front end.
        float width = mix(0.018, 0.72, pow(uv.y, 1.65));
        float center = 0.5;
        float distFromCenter = abs(uv.x - center);

        // Triangle mask that goes to a sharp infinity point.
        float triangle = 1.0 - smoothstep(width * 0.92, width, distFromCenter);

        // Fade the very point so it feels endless instead of like a hard cap.
        float farFade = smoothstep(0.0, 0.18, uv.y);
        float nearFade = 1.0 - smoothstep(0.94, 1.0, uv.y);
        float alpha = triangle * farFade * nearFade * uOpacity;

        // Horizontal-only drifting reflections.
        float a = sin(uv.x * 34.0 + uTime * 1.45 + uv.y * 3.0);
        float b = sin(uv.x * 78.0 - uTime * 2.05 + uv.y * 5.0);
        float c = sin(uv.x * 145.0 + uTime * 0.85);
        float water = a * 0.42 + b * 0.23 + c * 0.10;
        water = water * 0.5 + 0.5;

        vec3 blue = vec3(0.70, 0.94, 1.0);
        vec3 cyan = vec3(0.78, 0.98, 1.0);
        vec3 pink = vec3(1.0, 0.80, 0.97);
        vec3 lavender = vec3(0.86, 0.80, 1.0);
        vec3 white = vec3(1.0, 0.98, 1.0);

        vec3 color = mix(blue, pink, water);
        color = mix(color, cyan, 0.24);
        color = mix(color, lavender, uv.y * 0.18);

        float gloss = smoothstep(0.74, 1.0, water) * triangle;
  ...
  const water = new THREE.Mesh(geometry, material);
  water.name = 'InfinityTrianglePastelWater';

  // Higher up vertically and farther away: appears above the top end of the old water.
  water.rotation.x = -Math.PI / 2;
  water.position.set(0, -0.38, -28.0);
  water.renderOrder = 2;
...
export default function CosmicVoyage() {
  ...
  useEffect(() => {
    ...
    // NEW: higher triangular water extension above the far/top end.
    ...
    renderer.setAnimationLoop(() => {
      ...
      // Animate only the internal reflections of the new triangle water.
      // The triangle shape itself stays locked in place.

...
function createInfinityTriangleWater() {
  ...
  // It is rendered first like z-index: -100000, so the old opaque water,
  ...
  const vertices = new Float32Array([
    ...
    // Very far center point. Camera perspective makes it flatten to infinity.
      0.0, 0.0, -950.0,
  ]);
  ...
  const material = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        // Soft triangle edges. No transparency is used, so render order stays stable.
        float leftEdge = smoothstep(0.0, 0.065, uv.x);
        float rightEdge = smoothstep(1.0, 0.935, uv.x);
        float baseFade = smoothstep(0.0, 0.16, uv.y);
        float tipFade = 1.0 - smoothstep(0.88, 1.0, uv.y) * 0.18;
        float shape = leftEdge * rightEdge * baseFade * tipFade;
        ...
        vec3 blue = vec3(0.76, 0.93, 1.0);
        vec3 lavender = vec3(0.84, 0.80, 1.0);
        vec3 pink = vec3(1.0, 0.84, 0.98);
        vec3 white = vec3(1.0, 0.985, 1.0);

        vec3 color = mix(blue, lavender, uv.y * 0.75);
        color = mix(color, pink, water * 0.18);

        // Bright center road/reflection to sell the infinity point.
        ...
        // Fade into white at the sides/base without alpha.
        color = mix(bg, color, shape * 0.82);

...
function createFluffSpriteTexture() {
  ...
  gradient.addColorStop(0.58, 'rgba(255,255,255,0.20)');
  ...
  for (let i = 0; i < 180; i += 1) {
    ...
    const start = 7 + Math.random() * 12;
    const end = 26 + Math.random() * 32;
...
function shouldProtectFromFluff(object, skipPattern) {
...
function createTextureSafeFluffRimMaterial({
  ...
  return new THREE.ShaderMaterial({
    ...
    vertexShader: `
      ...
      void main() {
        ...
        float littleWiggle = sin(position.x * 9.0 + position.y * 7.0 + uTime * 2.4 + shell * 6.0) * 0.006;
        vec3 pushedPosition = position + normal * (uLength * shell + littleWiggle * shell);
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        float rim = pow(1.0 - facing, 0.38);
        float softPulse = 0.88 + sin(uTime * 2.2 + shell * 8.0) * 0.12;
        float alpha = rim * uOpacity * softPulse * (1.0 - shell * 0.35);
...
function addTextureSafeFluffRim(root, options = {}) {
  const {
    layers = 3,
    length = 0.055,
    opacity = 0.18,
    tint = '#fff0fb',
    skipPattern = /eye|eyes|pupil|iris|glass|window|light|glow|black|mouth|teeth|screen|gem|crystal/i,
  } = options;
  ...
  root.traverse((object) => {
    ...
    if (shouldProtectFromFluff(object, skipPattern)) return;
    ...
    for (let layer = 1; layer <= layers; layer += 1) {
      const material = createTextureSafeFluffRimMaterial({
        color: tintColor,
        layer,
        layers,
        length,
      ...
      shell.name = `${object.name || 'mesh'}_texture_safe_fluffy_rim_${layer}`;
      shell.frustumCulled = false;
      shell.renderOrder = 35 + layer;
      shell.userData.textureSafeFluff = true;
...
function collectTextureSafeSurfaceSamples(root, skipPattern) {
  const samples = [];
  ...
  root.traverse((object) => {
    ...
    if (object.userData?.textureSafeFluff) return;
    if (shouldProtectFromFluff(object, skipPattern)) return;
    ...
    const step = Math.max(1, Math.floor(positionAttr.count / 420));
...
function addTextureSafeFluffDots(root, options = {}) {
  const {
    texture,
    count = 360,
    size = 0.075,
  ...
  const samples = collectTextureSafeSurfaceSamples(root, skipPattern);
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    const out = shellOffset * (0.85 + Math.random() * 1.65);
    ...
    const color = colorPalette[i % colorPalette.length]
      .clone()
      .lerp(new THREE.Color('#ffffff'), Math.random() * 0.36);
  ...
  cloud.name = `${root.name || 'model'}_texture_safe_fluff_dots`;
  cloud.frustumCulled = false;
  cloud.renderOrder = 48;
  cloud.userData.fluffDots = {
    baseSize: size,
    baseOpacity: opacity,
...
new THREE.TextureLoader().load(
  ...
  (texture) => {
    ...
    // Texture-safe fluff: this adds only transparent rim shells and small outside dots.
    // It never replaces the original GLB materials, so the model textures stay visible.
    const fluffTexture = createFluffSpriteTexture();
    const fluffRimMaterials = [];
    const fluffDotClouds = [];
    ...
    Promise.all([
      ...
      .then(([boatGLTF, catGLTF]) => {
        ...
        fluffRimMaterials.push(
          ...addTextureSafeFluffRim(boatModel, {
            layers: 2,
            length: 0.035,
            opacity: 0.10,
            tint: '#ffe8ff',
        ...
        const boatFluffDots = addTextureSafeFluffDots(boatModel, {
          texture: fluffTexture,
          count: width < 768 ? 180 : 260,
          size: 0.045,
          opacity: 0.22,
          shellOffset: 0.055,
          jitter: 0.018,
          palette: ['#ffe8ff', '#e8ddff', '#d9f3ff'],
        });

        if (boatFluffDots) fluffDotClouds.push(boatFluffDots);
        ...
        fluffRimMaterials.push(
          ...addTextureSafeFluffRim(catModel, {
            layers: 3,
            length: 0.070,
            opacity: 0.16,
            tint: '#f7ecff',
        ...
        const catFluffDots = addTextureSafeFluffDots(catModel, {
          texture: fluffTexture,
          count: width < 768 ? 360 : 520,
          size: 0.080,
          opacity: 0.38,
          shellOffset: 0.090,
          jitter: 0.030,
          palette: ['#fff4fb', '#f4e7ff', '#e8fbff', '#ffffff'],
        });

        if (catFluffDots) fluffDotClouds.push(catFluffDots);
    ...
    renderer.setAnimationLoop(() => {
      ...
      fluffRimMaterials.forEach((material) => {
        material.uniforms.uTime.value = elapsed;
      });

      fluffDotClouds.forEach((cloud) => {
        const settings = cloud.userData.fluffDots;
        if (!settings) return;

        cloud.material.size = settings.baseSize * (0.92 + Math.sin(elapsed * 2.1 + settings.phase) * 0.08);
        cloud.material.opacity = settings.baseOpacity * (0.86 + Math.sin(elapsed * 1.7 + settings.phase) * 0.14);
      });

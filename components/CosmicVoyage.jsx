...
function createFluffSpriteTexture() {
  // Soft cotton/fur sprite, not a sparkle dot.
  // This texture is intentionally low contrast so it does not become
  // the white explosion shown in the screenshot.
  const size = 128;
  ...
  gradient.addColorStop(0.20, 'rgba(255,255,255,0.58)');
  gradient.addColorStop(0.50, 'rgba(255,255,255,0.24)');
...
function shouldSkipFluff(object, skipPattern) {
  if (!object.isMesh || !object.geometry?.attributes?.position) return true;
  if (object.userData?.softFluffCloud) return true;
  ...
  if (skipPattern.test(searchText)) return true;

  const positionAttr = object.geometry.attributes.position;
  if (positionAttr.count < 24) return true;
  ...
  // Tiny trim pieces become floating white dust, so skip them.
  return biggestSide < 0.12;
}

function collectFluffSurfaceSamples(root, skipPattern, maxSamplesPerMesh = 360) {
  const samples = [];
  ...
  root.traverse((object) => {
    ...
    const step = Math.max(
      1,
      Math.floor(positionAttr.count / maxSamplesPerMesh),
    );
...
function addTexturePreservingFluff(root, options = {}) {
  const {
    ...
    shellOffset = 0.060,
    jitter = 0.018,
    palette = ['#fff7ff', '#f5ebff', '#ecfbff', '#ffffff'],
    skipPattern = /eye|eyes|pupil|iris|glass|window|mouth|teeth|screen|metal|chrome|wheel|propeller|bolt|button|trim|inside|shadow/i,
  } = options;

  if (!texture) return null;

  let samples = collectFluffSurfaceSamples(root, skipPattern);

  // Safety fallback: if the GLB mesh names/material names are all skipped,
  // try again with almost no name filtering so the fluff does not vanish.
  if (!samples.length) {
    samples = collectFluffSurfaceSamples(root, /a^/);
  }

  if (!samples.length) {
    console.warn('No fluff samples found for', root.name || root);
    return null;
  }
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    // Keep the fur shell tight to the surface.
    // The old effect floated too far away and looked like white fireworks.
    const distance = shellOffset * (0.55 + Math.random() * 0.85);
    ...
    const color = colorPalette[i % colorPalette.length]
      .clone()
      .lerp(new THREE.Color('#ffffff'), Math.random() * 0.20);
  ...
  const material = new THREE.PointsMaterial({
    ...
    alphaTest: 0.025,
    depthWrite: false,
    // false keeps the soft fur halo visible on tiny mobile models.
    // NormalBlending + low opacity prevents the old white explosion look.
    depthTest: false,
  ...
  cloud.name = `${root.name || 'model'}_soft_surface_fluff`;
  cloud.frustumCulled = false;
  cloud.renderOrder = 58;
...
const tipMaterial = new THREE.SpriteMaterial({
  ...
  // Keeps the PNG from becoming gray/dim under ACES tone mapping.
  toneMapped: false,
  ...
  (error) => {
    ...
    Promise.all([
      ...
      .then(([boatGLTF, catGLTF]) => {
        ...
        const boatFluff = addTexturePreservingFluff(boatModel, {
          ...
          count: width < 768 ? 1800 : 2600,
          size: 0.065,
          opacity: 0.18,
          shellOffset: 0.060,
          jitter: 0.018,
          palette: ['#fff4fb', '#f2e9ff', '#eafcff', '#ffffff'],
          skipPattern: /eye|eyes|pupil|iris|glass|window|mouth|teeth|screen|metal|chrome|wheel|propeller|bolt|button|trim|inside|shadow/i,
        });
        ...
        const catFluff = addTexturePreservingFluff(catModel, {
          texture: fluffTexture,
          count: width < 768 ? 2400 : 3400,
          size: 0.075,
          opacity: 0.22,
          shellOffset: 0.065,
          jitter: 0.020,
          palette: ['#fff8ff', '#f5ebff', '#eaffff', '#ffffff'],
          skipPattern: /eye|eyes|pupil|iris|glass|window|mouth|teeth|screen|metal|chrome|bolt|button|trim|inside|shadow/i,
        });
    ...
    renderer.setAnimationLoop(() => {
      ...
      fluffClouds.forEach((cloud) => {
        ...
        // Tiny breathing shimmer only. Do not rotate the cloud,
        // because rotating it makes the fluff slide away from the model.
        const shimmer = Math.sin(elapsed * 1.25 + motion.phase);
        cloud.material.size = motion.baseSize * (1.0 + shimmer * 0.025);
        cloud.material.opacity = motion.baseOpacity * (0.96 + shimmer * 0.04);
      });

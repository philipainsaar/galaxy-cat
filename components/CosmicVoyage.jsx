...
function createFlatPastelWaterLayer({
  name,
  z,
  y,
  opacity = 0.42,
  ...
  const geometry = new THREE.PlaneGeometry(120, 120, 96, 96);
  ...
  const material = new THREE.ShaderMaterial({
    ...
    vertexShader: `
      ...
      void main() {
        ...
        // Almost flat water: tiny movement only, so the old waves still do the big motion.
        p.z += sin((uv.x * 18.0) + (uv.y * 2.0) + uTime * 1.2) * 0.012;
        p.z += sin((uv.x * 42.0) - uTime * 1.6) * 0.006;
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        // Horizontal-only pastel reflection drift.
        float a = sin((uv.x * 18.0) + uTime * 1.15 + uv.y * 1.2);
        float b = sin((uv.x * 38.0) - uTime * 1.55 + uv.y * 0.8);
        float c = sin((uv.x * 82.0) + uTime * 0.75);

        float water = a * 0.42 + b * 0.28 + c * 0.12;
        water = water * 0.5 + 0.5;

        vec3 cyan = vec3(0.78, 0.96, 1.0);
        vec3 blue = vec3(0.70, 0.88, 1.0);
        vec3 pink = vec3(1.0, 0.84, 0.97);
        vec3 lavender = vec3(0.88, 0.84, 1.0);
        vec3 white = vec3(1.0, 0.98, 1.0);

        vec3 color = mix(blue, pink, water);
        color = mix(color, cyan, 0.26);
        color = mix(color, lavender, smoothstep(0.25, 0.95, uv.y) * 0.18);

        float gloss = smoothstep(0.74, 1.0, water);
        color = mix(color, white, gloss * 0.42);

        // Soft transparent edge so this layer blends into the old water.
        float sideFade = smoothstep(0.0, 0.08, uv.x) * (1.0 - smoothstep(0.92, 1.0, uv.x));
        float depthFade = smoothstep(0.0, 0.10, uv.y) * (1.0 - smoothstep(0.88, 1.0, uv.y));
        float alpha = sideFade * depthFade * uOpacity;
...
export default function CosmicVoyage() {
  ...
  useEffect(() => {
    ...
    const backPastelWater = createFlatPastelWaterLayer({
      name: 'BackFlatPastelWater',
      z: -5.2,
      y: -1.34,
      opacity: 0.52,
      renderOrder: 0,
    ...
    pastelWater.water.renderOrder = 1;
    scene.add(pastelWater.group);

    const frontPastelWater = createFlatPastelWaterLayer({
      name: 'FrontFlatPastelWater',
      z: 2.6,
      y: -1.08,
      opacity: 0.58,
      renderOrder: 3,
    ...
    renderer.setAnimationLoop(() => {
      ...
      // Flat pastel water layers in front of and behind the original high waves.
      // These use shader UV motion, so they drift sideways without circles or crashing overlays.
      backPastelWater.material.uniforms.uTime.value = elapsed;
      frontPastelWater.material.uniforms.uTime.value = elapsed;
